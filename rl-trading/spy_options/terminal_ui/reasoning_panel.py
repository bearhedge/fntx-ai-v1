...
class ReasoningPanel:
    ...
    def create_panel(self,
        ...
        # 5. Confidence Analysis (moved down)
        confidence_section = self._create_confidence_section(
            action, action_probs, features
        )
        sections.append(confidence_section)
        
        # 6. Recent History (moved down)
    ...
    def _create_rl_status_section(self, constraints: Dict) -> Panel:
        ...
        if status == 'active':
            ...
            # Prediction timing
            prediction_time = constraints.get('prediction_time', 'N/A')
            table.add_row(
                "Prediction Time:",
                f"[dim]{prediction_time}[/dim]"
            )
            
            # Next update countdown
            next_update = constraints.get('next_update_in', 'N/A')
            table.add_row(
                "Next Update:",
                f"[cyan]{next_update}[/cyan]"
            )
        elif status == 'waiting':
            # Waiting for next bar
            if constraints.get('waiting_for_next_bar'):
                next_bar_in = constraints.get('next_bar_in', 'N/A')
                table.add_row(
                    "Next Bar In:",
                    f"[cyan]{next_bar_in}[/cyan]"
                )
                table.add_row(
                    "Status:",
                    "[dim]Waiting for 5-min bar[/dim]"
                )
