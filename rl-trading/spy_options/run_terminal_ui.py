...
async def main():
    ...
    # Get project root and load environment variables from .env file
    project_root = Path(__file__).parent.parent.parent
    ...
    parser.add_argument('--model', type=str,
                       default='models/gpu_trained/ppo_gpu_test_20250706_074954.zip',
    ...
    # Always try IBKR first for real-time data
    print("\n" + "="*60)
    print("Fetching Account Balance")
    print("="*60)
    
    # Step 1: Try IBKR FlexQuery first
    try:
        import os
        if os.getenv("IBKR_FLEX_TOKEN") and os.getenv("IBKR_FLEX_QUERY_ID"):
            print("Attempting to fetch from IBKR FlexQuery...")
            
            # Import FlexQuery service
            if str(project_root) not in sys.path:
                sys.path.insert(0, str(project_root))
            if str(project_root / "01_backend") not in sys.path:
                sys.path.insert(0, str(project_root / "01_backend"))
            
            from services.ibkr_flex_query_enhanced import flex_query_enhanced
            
            # Request and retrieve the report
            reference_code = flex_query_enhanced.request_flex_report()
            if reference_code:
                import time
                print("Waiting for report generation...")
                time.sleep(5)  # Wait briefly for report
                
                xml_data = flex_query_enhanced.get_flex_report(reference_code, max_retries=5)
                if xml_data:
                        # Debug: Save raw XML for inspection
                        import datetime as dt
                        debug_filename = f"debug_flexquery_{dt.datetime.now().strftime('%Y%m%d_%H%M%S')}.xml"
                        with open(debug_filename, 'w', encoding='utf-8') as f:
                            f.write(xml_data)
                        print(f"  DEBUG: Saved raw XML to {debug_filename}")
                        
                        # Parse account info - look for HKD data first
                        import xml.etree.ElementTree as ET
                        root = ET.fromstring(xml_data)
                        
                        
                        # Try to find NAV data in ChangeInNAV for the most recent period
                        all_nav_elems = root.findall(".//ChangeInNAV")
                        if all_nav_elems:
                            latest_elem = sorted(all_nav_elems, key=lambda x: x.get('toDate', ''), reverse=True)[0]
                            actual_capital = float(latest_elem.get('endingValue', '0'))
                            base_currency = latest_elem.get('currency', 'USD')
                            capital_source = "IBKR (Live)"
                            print(f"✓ IBKR Account Value: {actual_capital:,.2f} {base_currency}")
                            
                            # Get cash balance in the same currency
                            cash_elem = root.find(f".//CashReportCurrency[@currency='{base_currency}']")
                            if cash_elem:
                                ending_cash = float(cash_elem.get('endingCash', '0'))
                                print(f"  Cash Balance: {ending_cash:,.2f} {base_currency}")
                            
                            # Parse active SPY option positions from Trades
                            print("\nActive SPY Options:")
                            active_positions = {}
                            
                            # Check for closing trades that might be exercises
                            print("\nChecking SPY option trades for potential exercises:")
                            
                            for trade in root.findall(".//Trade[@symbol='SPY'][@assetCategory='OPT']"):
                                strike = trade.get('strike')
                                right = trade.get('putCall', '')
                                oc_indicator = trade.get('openCloseIndicator', '')
                                notes = trade.get('notes', '')
                                trans_type = trade.get('transactionType', '')
                                trade_date = trade.get('tradeDate', '')
                                
                                # Show closing trades that might be exercises
                                if oc_indicator == 'C' or 'exercise' in notes.lower() or 'assignment' in notes.lower():
                                    print(f"  Potential exercise - Strike: {strike}{right[0]}, OC: {oc_indicator}, Notes: {notes}, Type: {trans_type}, Date: {trade_date}")
                            
                            for trade in root.findall(".//Trade[@symbol='SPY'][@assetCategory='OPT'][@openCloseIndicator='O']"):
                                strike = trade.get('strike')
                                right = trade.get('putCall', '')
                                ...
                                # Create position key
                                key = f"{strike}{right[0] if right else ''}"
                                if key not in active_positions:
                                    active_positions[key] = 0
                                active_positions[key] += quantity
                            
                            # Display active positions
                            if active_positions:
                                for pos_key, qty in active_positions.items():
                                    if qty != 0:  # Only show non-zero positions
                                        print(f"  - {pos_key}: {qty} contracts")
                            else:
                                print("  No active SPY option positions found")
                            
                            # Store positions for dashboard use
                            ibkr_positions = {k: v for k, v in active_positions.items() if v != 0}
                            
                            # Parse Option Exercises from FlexQuery
                            print("\nChecking Exercises in FlexQuery:")
                            exercises_found = []
                            
                            # Show report date range to understand data coverage
                            print("\nChecking FlexQuery date range and account:")
                            ...
                            # Check OptionEAE (Option Exercises, Assignments and Expirations) section
                            all_exercises = root.findall(".//OptionEAE")
                            if all_exercises:
                                print(f"\n  Found {len(all_exercises)} total OptionEAE records")
                                
                                # Quick scan to categorize records
                                spy_options = []
                                spy_stocks = []
                                other_records = []
                                ...
                                for exercise in all_exercises:
                                    ...
                                    if asset_cat == 'OPT' and 'SPY' in symbol.upper():
                                        spy_options.append(exercise)
                                    elif asset_cat == 'STK' and symbol == 'SPY':
                                        spy_stocks.append(exercise)
                                    else:
                                        other_records.append(exercise)
                                
                                # Summary table
                                print("\n  ┌─────────────────────────────────────────────┐")
                                print("  │         OptionEAE Records Summary           │")
                                print("  ├─────────────────────────────────────────────┤")
                                print(f"  │ SPY Options:     {len(spy_options):3d} records                │")
                                print(f"  │ SPY Stock:       {len(spy_stocks):3d} records                │")
                                print(f"  │ Other:           {len(other_records):3d} records                │")
                                print(f"  │ Total:           {len(all_exercises):3d} records                │")
                                print("  └─────────────────────────────────────────────┘")
                            
                            # Parse SPY options only
                            print("\n  Processing SPY Options:")
                            all_optionae = root.findall(".//OptionEAE")
                            print(f"  DEBUG: Found {len(all_optionae)} total OptionEAE elements")
                            
                            for exercise in all_optionae:
                                asset_cat = exercise.get('assetCategory', '')
                                symbol = exercise.get('symbol', '')
                                
                                # Skip non-SPY options without verbose output
                                if not (asset_cat == 'OPT' and 'SPY' in symbol.upper()):
                                    continue
                                
                                # Handle empty records
                                if not symbol and not asset_cat:
                                    continue
                                    
                                if asset_cat == 'OPT' and symbol and 'SPY' in symbol.upper():
                            ...
                            # Check Pending Exercises section (try both singular and plural)
                            all_pending = root.findall(".//PendingExercises")
                            all_pending_singular = root.findall(".//PendingExercise")
                            
                            print(f"\n  Checking PendingExercises sections:")
                            print(f"    Found {len(all_pending)} PendingExercises elements")
                            print(f"    Found {len(all_pending_singular)} PendingExercise elements")
                            
                            # Try both forms
                            pending_elements = all_pending + all_pending_singular
                            if pending_elements:
                                print("\n  DEBUG - ALL Pending Exercise records:")
                                for i, pending in enumerate(pending_elements, 1):
                                    print(f"\n  Pending Record #{i}:")
                                    for attr, value in pending.attrib.items():
                                        print(f"    {attr}: {value}")
                            
                            for pending in pending_elements:
                                # Check if SPY related
                                symbol = pending.get('symbol', '')
                                asset_cat = pending.get('assetCategory', '')
                                
                                if asset_cat == 'OPT' and 'SPY' in symbol.upper():
                                    strike = pending.get('strike')
                                    put_call = pending.get('putCall', '')
                                    date = pending.get('date', 'PENDING')
                                    quantity = pending.get('quantity', '1')
                                    
                                    symbol_short = f"{strike}{put_call[0] if put_call else ''}"
                                    print(f"  - PENDING EXERCISE: {symbol_short} on {date}")
                                    exercises_found.append({
                                        'symbol': symbol_short,
                                        'date': date,
                                        'type': 'Pending',
                            ...
                            if exercises_found:
                                print(f"\n  DEBUG: exercises_found contains {len(exercises_found)} items:")
                                for i, ex in enumerate(exercises_found):
                                    print(f"    {i+1}. {ex['date']} - {ex['symbol']} - {ex['type']}")
                                ...
                                for date in sorted(by_date.keys()):
                                    ...
                                    if events['assignments']:
                                        ...
                                        # Log assignments for RL training
                                        for symbol in events['assignments']:
                                            try:
                                                strike = float(symbol[:-1])
                                                position_id = f"{symbol}_{date}"
                                                # Get current SPY price from data connector if available
                                                current_spy_price = 0
                                                if 'data_connector' in locals() and hasattr(data_connector, 'get_current_snapshot'):
                                                    try:
                                                        snapshot = data_connector.get_current_snapshot()
                                                        current_spy_price = snapshot.get('spy_price', 0)
                                                    except:
                                                        pass
                                                exercise_logger.log_exercise(position_id, current_spy_price)
                                            except ValueError:
                                                pass
                                ...
                                print("\n  ✓ No SPY option events found")
                            
                            # Store for dashboard
                            ibkr_exercises = exercises_found
                else:
                    print("❌ Failed to retrieve IBKR report")
            else:
                print("❌ Failed to request IBKR report")
        else:
            print("⚠️  IBKR credentials not set in environment")
    except Exception as e:
        print(f"⚠️  Error fetching IBKR balance: {e}")
    
    # Step 2: If IBKR failed, try database as fallback
    if actual_capital is None and args.use_database:
        ...
        try:
            ...
            if conn:
                with conn.cursor() as cursor:
                    # Get latest NAV snapshot from portfolio.daily_nav_snapshots
                    cursor.execute("""
                        SELECT snapshot_date, closing_nav, source
                    ...
                    if result and result[1] is not None:
                        snapshot_date, closing_nav, source = result
                        actual_capital = float(closing_nav)
                        # Try to detect currency from database or default to USD
                        if base_currency is None:
                            base_currency = "USD"  # Default if not detected from IBKR
                        capital_source = f"Database (as of {snapshot_date})"
                        print(f"✓ Database NAV: {actual_capital:,.2f} {base_currency}")
                        print(f"  Last updated: {snapshot_date}")
                    else:
                        print("⚠️  No NAV snapshots in database")
                conn.close()
        except Exception as e:
    ...
    if actual_capital is None:
        ...
        print(f"{'='*60}")
        print("\nTo fix this:")
        print("  1. Set IBKR_FLEX_TOKEN and IBKR_FLEX_QUERY_ID in .env")
        print("  2. Run daily_flex_import.py to populate database")
        print("  3. Use --force-ibkr-refresh to force IBKR fetch")
        print(f"\n{'='*60}\n")
        sys.exit(1)
