#!/usr/bin/env python3
"""
Simple working downloader without conflicts
"""
import sys
import requests
import psycopg2
...
class SimpleDownloader:
    def __init__(self):
        ...
        self.delay = 0.3
        self.interval = 300000  # 5 minutes
        self.report = {
            'successful_days': [],
            'failed_days': [],
            'total_contracts': 0,
    ...
    def save_contract_data(self, cursor, trade_date: datetime, strike: int, right: str,
        ...
        # Insert contract (simple approach)
        cursor.execute("""
        ...
        # Save Greeks data (exclude 16:00)
        ...
        # Save IV data with CORRECT parsing
        for iv in iv_data:
            ts = self.parse_timestamp(iv[0], exp_int)
            
            # CORRECT IV parsing: Calls use field [4], Puts use field [2]
            if right == 'C':
    ...
    def download_day(self, trade_date: datetime) -> Dict:
        """Download 0DTE options for a single day"""
    ...
    def download_december_2022(self):
        ...
        print("DECEMBER 2022 SPY 0DTE OPTIONS - SIMPLE WORKING DOWNLOAD")
        print("Downloading OHLC + Greeks + IV with 5-minute intervals")
        print("Using daily opening price ± $20 for strike selection")
        print("With CORRECT IV parsing: Calls field [4], Puts field [2]")
    ...
    def print_report(self):
        ...
        print("\n" + "="*100)
        print("DECEMBER 2022 SPY 0DTE DOWNLOAD REPORT - WITH CORRECT IV PARSING")
        print("="*100)
        ...
        if self.report['successful_days']:
            ...
            print(f"\n✅ Expected ~100% IV coverage with correct parsing!")
