#!/usr/bin/env python3
"""
Hourly Options Downloader
Downloads options data with hourly granularity (6-7 bars per day)
Based on Foolproof V2 with automatic strike interval detection
"""
import sys
import os
import requests
...
def setup_logging():
    ...
    log_file = log_dir / f'hourly_downloader_{datetime.now().strftime("%Y%m%d_%H%M%S")}.log'
...
class HourlyOptionsDownloader:
    """Downloads options data with hourly granularity for faster processing"""
    ...
    def __init__(self, max_workers: int = 10):
        ...
        self.checkpoint_file = Path('/home/info/fntx-ai-v1/08_logs/hourly_checkpoint.json')
    ...
    def _download_single_contract(self, contract_params: Dict) -> Optional[Tuple[int, List]]:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if data.get('response'):
                    bars = data['response']
                    self.logger.debug(f"Got {len(bars)} hourly bars for {contract_params['strike']/1000}{contract_params['right']}")
    ...
    def process_trading_day(self, date: datetime) -> bool:
        ...
        for exp_str in expirations:
            ...
            self.logger.info(f"⏰ Using hourly bars (expecting ~6-7 bars per contract)")
    ...
    def run_date_range(self, start_date: datetime, end_date: datetime):
        ...
        self.logger.info(f"\n{'='*80}")
        self.logger.info(f"🚀 HOURLY OPTIONS DOWNLOADER")
        self.logger.info(f"📅 Period: {start_date.strftime('%Y-%m-%d')} to "
                        f"{end_date.strftime('%Y-%m-%d')}")
        self.logger.info(f"📊 Strategy: Open-Close Union with ±5% buffer")
        self.logger.info(f"🔧 Features: Hourly bars (6-7 per day), Auto strike interval detection")
    ...
    def _print_final_statistics(self):
        ...
        if self.stats['dates_processed'] > 0:
            ...
            avg_records = self.stats['total_records'] / self.stats['dates_processed']
            self.logger.info(f"\n📊 Averages per day:")
            self.logger.info(f"   Contracts: {avg_contracts:.1f}")
            self.logger.info(f"   Unique strikes: {avg_strikes:.1f}")
            self.logger.info(f"   Total hourly bars: {avg_records:.1f}")
            if self.stats['contracts_downloaded'] > 0:
                bars_per_contract = self.stats['total_records'] / self.stats['contracts_downloaded']
                self.logger.info(f"   Hourly bars per contract: {bars_per_contract:.1f}")


def main():
    """Run January 2020 download with hourly data"""
    # Setup logging
    setup_logging()
    ...
    downloader = HourlyOptionsDownloader(max_workers=2)
