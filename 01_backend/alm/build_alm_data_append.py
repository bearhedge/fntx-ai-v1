...
DEFAULT_DATA_DIR = "/home/info/fntx-ai-v1/04_data"
HKD_USD_RATE = 7.8472
...
def check_itm_options_for_assignment(trades_tree, exercises_tree, target_date):
    ...
    for symbol, (net_qty, strike, put_call, expiry, close_price) in short_options.items():
        ...
        # For OTM options that closed at zero, don't assume assignment
        ...
        if is_itm or near_money:
            ...
            event_ts = datetime.strptime(f"{target_date_str} 16:20:00", "%Y%m%d %H:%M:%S")
            ...
            if put_call == 'C':
                ...
                description = f"{symbol} ASSUMED Assignment @ ${strike} (ITM Call, SPY@${spy_close_price:.2f})"
                ...
                description = f"{symbol} ASSUMED Assignment @ ${strike} (ITM Put, SPY@${spy_close_price:.2f})"
            ...
            if put_call == 'C':
                ...
                stock_event = {
                    'event_timestamp': event_ts,
                    'event_type': 'Trade',
                    'description': f"SPY ASSUMED Delivery (Call Assignment) @ ${strike}",
                ...
                stock_event = {
                    'event_timestamp': event_ts,
                    'event_type': 'Trade',
                    'description': f"SPY ASSUMED Receipt (Put Assignment) @ ${strike}",
                    'cash_impact_hkd': -stock_qty * strike_float * HKD_USD_RATE,
                    'realized_pnl_hkd': 0,
                    'ib_commission_hkd': 0,
                    'source_transaction_id': f"ASSUMED_STOCK_{symbol}_{target_date_str}"
...
def parse_and_insert_data(db_url: str, append: bool, data_dir: str = None):
    ...
    for trade in all_trades:
        ...
        if date_time:
            ...
            event_ts = datetime.strptime(date_time.split()[0], "%Y%m%d;%H%M%S")
        else:
            trade_time = attribs.get('tradeTime', '16:00:00')
            event_ts = datetime.strptime(f"{trade_date} {trade_time}", "%Y%m%d %H:%M:%S")
    ...
    for statement in cash_tree.findall(".//FlexStatement"):
        ...
        for trans in cash_trans:
            ...
            # Use report date with default time for cash transactions
            event_ts = datetime.strptime(f"{report_date} 16:00:00", "%Y%m%d %H:%M:%S")
        ...
        try:
            ...
            for statement in exercises_tree.findall(".//FlexStatement"):
                ...
                for i, opt_event in enumerate(option_events):
                    ...
                    event_ts = datetime.strptime(f"{event_date} 16:00:00", "%Y%m%d %H:%M:%S")
        ...
        try:
            ...
            for statement in interest_tree.findall(".//FlexStatement"):
                ...
                for accrual in statement.findall(".//InterestAccrual"):
                    ...
                    event_ts = datetime.strptime(f"{to_date} 16:00:00", "%Y%m%d %H:%M:%S")
