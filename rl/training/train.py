"""
Main training script for SPY Options RL Agent
Trains baseline PPO agent with comprehensive logging for RLHF
"""
import argparse
import sys
...
from config import DB_CONFIG, RL_CONFIG
from data import OptionsDataLoader
from environments import SPY0DTEEnvironment
from agents import SimplePnLReward, OptionsEpisodeReward
...
def create_training_env(data_loader: OptionsDataLoader, 
...
def create_eval_env(data_loader: OptionsDataLoader) -> SPY0DTEEnvironment:
...
def train_baseline_agent(
    ...
    train_start_date = train_start_date or RL_CONFIG['train_start_date']
    train_end_date = train_end_date or RL_CONFIG['train_end_date']
    
    logger.info(f"Training from {train_start_date} to {train_end_date}")
    
    # Initialize data loader
    data_loader = OptionsDataLoader(DB_CONFIG)
    logger.info(f"Loaded {len(data_loader.trading_days)} trading days")
    
    # Filter training days
    train_days = [
        d for d in data_loader.trading_days
        if train_start_date <= d.strftime('%Y-%m-%d') <= train_end_date
    ]
    logger.info(f"Using {len(train_days)} days for training")
    ...
    try:
        ...
        # Save final model
        final_path = f"./models/final/ppo_baseline_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Train PPO agent for SPY options trading"
    )
