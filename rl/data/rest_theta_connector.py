                ...
                for contract_data in ohlc['response']:
                    if 'contract' in contract_data:
                        ...
                        if option_key in self.market_data['options_chain']:
                            ticks = contract_data.get('ticks', [])
                            if ticks and len(ticks[0]) >= 6:  # Volume is at position 5, so need at least 6 fields
                                # Format: [ms_of_day, open, high, low, close, volume, count, date]
                                volume = ticks[0][5] if len(ticks[0]) > 5 else 0
                                self.market_data['options_chain'][option_key]['volume'] = volume
                                if volume > 0:
                                    self.logger.debug(f"Updated volume for {option_key}: {volume}")
                    ...
                    if 'contract' in contract_data:
                        ...
                        if option_key in self.market_data['options_chain']:
                            ...
                            if ticks and len(ticks[0]) >= 5:
                                # Format: [ms_of_day, bid, ask, delta, gamma, theta, vega, rho, epsilon, iv, iv_error, ms_of_day2, underlying_price, date]
                                tick_data = ticks[0]
                                
                                # Get Greeks from correct fields
                                ...
                                # Greeks are already in correct format from Theta Terminal
                                self.market_data['options_chain'][option_key]['delta'] = delta
                                self.market_data['options_chain'][option_key]['gamma'] = gamma
                                self.market_data['options_chain'][option_key]['theta'] = theta
                                self.market_data['options_chain'][option_key]['iv'] = iv
                                
                                self.logger.debug(f"Updated Greeks for {option_key}: delta={delta:.3f}, iv={iv:.1%}")
