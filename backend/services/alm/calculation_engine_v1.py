#!/usr/bin/env python3
"""
ALM Daily Performance Report Generator - Version 1.0
Produces detailed analysis of trading activity and P&L

Version History:
- V1.0 (2025-07-25): Fixed timezone display to show "(next day)" when disposals cross date boundary
                     Correctly handles EDT to HKT conversion with date awareness
"""
...
def generate_summary_table(cursor):
    ...
    for summary_date, opening_nav, closing_nav, gross_pnl, net_cash_flow in summaries:
        ...
        # Check for deposits before market open
        est = pytz.timezone('US/Eastern')
        market_open = est.localize(datetime.combine(summary_date, datetime.min.time()) + timedelta(hours=9, minutes=30))
        
        cursor.execute("""
            SELECT SUM(cash_impact_hkd) 
            FROM alm_reporting.chronological_events
            WHERE DATE(event_timestamp AT TIME ZONE 'US/Eastern') = %s
            AND event_type = 'Deposits/Withdrawals'
            AND cash_impact_hkd > 0
            AND event_timestamp < %s
        """, (summary_date, market_open))
        
        result = cursor.fetchone()
        deposits_before_open = result[0] if result and result[0] else Decimal(0)
        
        # Calculate return using net P&L divided by appropriate NAV base
        if deposits_before_open > 0:
            adjusted_opening_nav = opening_nav + deposits_before_open
            net_pnl_pct = (net_pnl / adjusted_opening_nav * 100) if adjusted_opening_nav != 0 else 0
        else:
...
def generate_daily_narrative(cursor, summary_date):
    ...
    cursor.execute("""
        SELECT event_timestamp, event_type, description, cash_impact_hkd, realized_pnl_hkd, ib_commission_hkd,
               COALESCE(is_synthetic, false) as is_synthetic
        FROM alm_reporting.chronological_events
        WHERE event_timestamp >= %s AND event_timestamp < %s
    ...
    for event in events:
        ts, etype, desc, cash, pnl = event[:5]
        commission = event[5] if len(event) > 5 else 0
        
        if etype == 'Trade' and is_option_symbol(desc) and 'Assigned' not in desc:
            option_type, strike = parse_option_details(desc)
            if option_type:
                # Extract quantity from description (e.g., "Sell 3 SPY..." -> 3)
                ...
                if cash > 0:
                    ...
                    trade_count += 1
                    contracts_opened.append((ts, etype, desc, cash, pnl, contract_desc, quantity, commission))
    ...
    # Check for deposits/withdrawals on this day
    est = pytz.timezone('US/Eastern')
    market_open = est.localize(datetime.combine(summary_date, datetime.min.time()) + timedelta(hours=9, minutes=30))
    market_close = est.localize(datetime.combine(summary_date, datetime.min.time()) + timedelta(hours=16))
    
    cursor.execute("""
        SELECT event_timestamp, description, cash_impact_hkd
        FROM alm_reporting.chronological_events
        WHERE DATE(event_timestamp AT TIME ZONE 'US/Eastern') = %s
        AND event_type = 'Deposits/Withdrawals'
        ORDER BY event_timestamp
    ...
    deposits_before_open = Decimal(0)
    withdrawals_after_close = Decimal(0)
    
    for ts, desc, amount in cash_flows:
        if amount > 0 and ts < market_open:
            deposits_before_open += amount
        elif amount < 0 and ts >= market_close:
            withdrawals_after_close += amount
    
    # Generate narrative with better formatting
    print(f"\n**Opening Position**")
    
    # Show adjusted NAV if there are deposits before market open
    if deposits_before_open > 0:
    ...
    if trade_count > 0:
        ...
        if contracts_opened:
            ...
            for ts, etype, desc, cash, pnl, contract_desc, quantity, commission in contracts_opened:
                option_type, strike = parse_option_details(desc)
                if option_type:
                    ...
                    print(f"      • Sold {quantity} SPY {expiry_formatted} ${strike:.0f} {contract_type}")
                    print(f"        - Premium received: {format_hdk(cash)}")
                    if commission:
                        print(f"        - Commission: {format_hdk(abs(commission))}")
                    print(f"        - Execution time: {ts.astimezone(HKT).strftime('%I:%M %p HKT')}")
                    
                    total_premium += Decimal(str(cash))
                    total_commission += Decimal(str(abs(commission))) if commission else Decimal(0)
            
            # Add totals section
            if len(contracts_opened) > 1:
                print(f"\n   **Trade Totals:**")
                print(f"      - Total Premium: {format_hdk(total_premium)}")
                print(f"      - Total Commissions: {format_hdk(total_commission)}")
                print(f"      - Net Premium: {format_hdk(total_premium - total_commission)}")
    ...
    # Get commission using the same function as the table
    commission_from_table = get_daily_commission(cursor, summary_date)
    
    # Calculate net P&L (gross P&L minus commissions) - same as table
    net_pnl = total_pnl - commission_from_table
    
    # Calculate return using NET P&L divided by appropriate NAV base - same as table
    # If deposits occurred before market open, use adjusted NAV for return calculation
    if deposits_before_open > 0:
        # Use net P&L divided by adjusted opening NAV
        nav_change_pct = (net_pnl / adjusted_opening_nav * 100) if adjusted_opening_nav != 0 else 0
    else:
        # Use net P&L divided by original opening NAV
        nav_change_pct = (net_pnl / opening_nav * 100) if opening_nav != 0 else 0
    ...
    # Commission impact - use the same commission value used for return calculation
    if commission_total > 0:
        print(f"   Total Commissions: {format_hdk(commission_total)}")
        print(f"   Net P&L: {format_hdk(net_pnl)}")  # net_pnl already calculated above
...
def enhance_trade_description(desc, pnl=None, cash=None, is_expiration=False, is_synthetic=False):
    """Enhance trade descriptions with context and proper formatting."""
    # Add [SYNTHETIC] marker if event is synthetic
    ...
    if is_expiration:
        ...
        if match:
            ...
            return f"{option_symbol} expired worthless{synthetic_marker}"
    ...
    if 'assigned' in desc.lower():
        ...
        if opt_match:
            ...
            if strike_match:
                ...
                return f"SPY {strike:.0f} {option_type} assigned (100 shares {'short' if option_type == 'Call' else 'long'} at ${strike:.2f}){synthetic_marker}"
        # Stock assignment
        else:
...
def generate_professional_narrative(cursor, summary_date, previous_closing_nav):
    ...
    if pre_market_events:
        ...
        for event in pre_market_events:
            ts, etype, desc, cash, pnl, comm, is_assign, is_synthetic = event
            ...
            print(f"    * **{ts.astimezone(hkt).strftime('%H:%M HKT')}:** {enhance_trade_description(desc, pnl, cash, is_expiration, is_synthetic)}")
            ...
            if len(group) > 1 and all(e[1] == 'Trade' for e in group):
                ...
                for event in group:
                    ...
                    is_expiration = (etype == 'Expiration' or 
                                   (ts.astimezone(est).hour == 16 and ts.astimezone(est).minute == 20 and 
                                    is_option_symbol(desc) and (cash or 0) == 0 and (pnl or 0) > 0 and 'Buy' in desc))
                    print(f"        - {enhance_trade_description(desc, pnl, cash, is_expiration, is_synthetic)}")
...
def main():
    ...
    # Suppress IB Gateway and connection logs for clean output
    ...
    if conn:
        with conn.cursor() as cursor:
            # Generate summary table (includes all dates from backend.data.database)
