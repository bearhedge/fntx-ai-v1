...
cd "$PROJECT_ROOT" && source 11_venv/bin/activate && PYTHONPATH="$PROJECT_ROOT" python3 -m uvicorn backend.api.main:app --host 0.0.0.0 --port 8000 --reload > 08_logs/api_server.log 2>&1 &
API_PID=$!

# Start EnvironmentWatcher Agent
echo -e "${BLUE}Starting EnvironmentWatcher Agent...${NC}"
cd "$PROJECT_ROOT" && source 11_venv/bin/activate && PYTHONPATH="$PROJECT_ROOT" python3 -m backend.agents.environment_watcher > 08_logs/environment_watcher.log 2>&1 &
ENV_PID=$!
...
cd "$PROJECT_ROOT/02_frontend" && npm run dev > ../08_logs/frontend.log 2>&1 &
FRONTEND_PID=$!

# Save PIDs for cleanup
echo "$API_PID" > "$PROJECT_ROOT/10_runtime/pids/api_server.pid"
echo "$ENV_PID" > "$PROJECT_ROOT/10_runtime/pids/environment_watcher.pid"
echo "$FRONTEND_PID" > "$PROJECT_ROOT/10_runtime/pids/frontend.pid"
...
echo -e "\n${YELLOW}Logs are available in the '08_logs' directory${NC}"
echo -e "${YELLOW}Run './06_scripts/dev/stop-dev.sh' to stop all services${NC}"
...
# Follow logs
tail -f "$PROJECT_ROOT/08_logs"/*.log 2>/dev/null